...
class Module(core.module.Module):
    ...
    def state(self, widget):
        strength = self.__compute_strength(self.__signal)
        # strength is None when the signal can't be read (e.g. wifi not yet
        # associated after a reconnect/resume) — treat as unknown, not fatal
        if strength is None:
            return None
        if strength < 50:
            return "critical"
